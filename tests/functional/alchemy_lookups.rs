...
use blockexplorer_tui::{
    adapters::rpc::{
        AlchemyAddressLookup, AlchemyBlockLookup, AlchemyEnsResolver, AlchemyTxLookup,
        RpcClient,
    },
    application::ports::{
        AddressLookupPort, BlockLookupPort, EnsResolverPort, TxLookupPort,
    },
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_getTransactionByHash__found.json"), "application/json"),
        )
        .mount(&server)
        .await;
...
async fn tx_lookup_returns_none_when_not_found() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_getTransactionByHash__not_found.json"), "application/json"),
        )
        .mount(&server)
        .await;
...
async fn block_lookup_by_hash_returns_summary() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_getBlockByHash__found.json"), "application/json"),
        )
        .mount(&server)
        .await;
...
async fn block_lookup_by_hash_returns_none_when_missing() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_getBlockByHash__not_found.json"), "application/json"),
        )
        .mount(&server)
        .await;
    ...
    let got = adapter.get_by_hash(hash, Chain::Ethereum).await.expect("ok");
    assert!(got.is_none());
}
...
async fn address_lookup_reports_contract_when_code_is_non_empty() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_getCode__contract.json"), "application/json"),
        )
        .mount(&server)
        .await;
...
async fn ens_forward_resolves_a_name_via_two_eth_calls() {
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_call__ens_resolver.json"), "application/json"),
        )
        .mount(&server)
        .await;
    ...
    Mock::given(method("POST"))
        ...
        .respond_with(
            ResponseTemplate::new(200)
                .set_body_raw(load_text("rpc__eth_call__ens_addr.json"), "application/json"),
        )
        .mount(&server)
        .await;
