...
async fn main() -> anyhow::Result<()> {
    ...
    // The probe uses the metadata-only variant: the Prices API is
    // called separately later in this example if needed. See
    // `plan/15-backlog.md` §3.4.
    match load_token_overview::run_metadata_only(&token_reader, token_addr, chain).await {
        Ok(t) => {
            println!(
            ...
            match t.price {
                PriceLookup::Available(p) => {
                    println!("price      : ${:.6} (currency {})", p.value, p.currency)
                }
                PriceLookup::Unsupported { provider } => {
                    println!("price      : (not indexed by {provider})")
                }
                PriceLookup::Pending => println!("price      : (pending)"),
            }
        }
        Err(e) => println!("overview ERROR: {e}"),
    }
